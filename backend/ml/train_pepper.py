"""Train a MobileNetV2 model for Pepper (Bell Pepper) disease classification.

Dataset: PlantVillage Pepper classes under <repo>/PlantVillage/PlantVillage/
...
Outputs (backend/ml/artifacts/):
    - pepper_model.pt          : TorchScript model
    - pepper_class_index.json  : class names in model output order
    - pepper_meta.json         : training metadata + metrics

Usage:
    cd backend && venv/bin/python ml/train_pepper.py
"""
...
MODEL_FILENAME = "pepper_model.pt"
CLASS_INDEX_FILENAME = "pepper_class_index.json"
META_FILENAME = "pepper_meta.json"
...
# ---------------------------------------------------------------------------
# Helpers (same pattern as train_tomato.py — kept here for standalone use)
# ---------------------------------------------------------------------------
...
def main():
    set_seed(SEED)
    print(f"[train_pepper] dataset root: {DATASET_ROOT}")
    print(f"[train_pepper] artifacts dir: {ARTIFACTS_DIR}")

    crop_classes = discover_classes(DATASET_ROOT, CROP_PREFIX)
    print(f"[train_pepper] discovered {len(crop_classes)} {CROP_PREFIX} classes:")
    ...
    print(f"[train_pepper] total images: {len(filtered)}")

    train_samples, val_samples = stratified_split(
        filtered.samples, val_fraction=VALIDATION_SPLIT, seed=SEED,
    )
    print(f"[train_pepper] train: {len(train_samples)}  val: {len(val_samples)}")
    ...
    device = get_device()
    print(f"[train_pepper] device: {device}")

    num_classes = len(crop_classes)
    print(f"[train_pepper] building MobileNetV2 with {num_classes} classes")
    ...
    meta = {
        ...
        "limitations": [
            ...
            "Covers 2 pepper leaf classes — does NOT cover cotton, soybean, "
    ...
    print(f"[train_pepper] saved metadata -> {meta_path}")
